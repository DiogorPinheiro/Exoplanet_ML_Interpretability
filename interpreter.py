...
from utilities import recall_m, f1_m, precision_m, auc_roc
...
if __name__ == "__main__":
    ...
    num_features = 10
    num_slices = 24
    series = global_X.iloc[idx, :]

    explainer = LimeTimeSeriesExplanation(
        class_names=['0', '1'], feature_selection='auto')
    exp = explainer.explain_instance(series, model.predict_proba, num_features=num_features, num_samples=5000, num_slices=num_slices,
                                     replacement_method='total_mean', training_set=global_X)
    exp.as_list()
